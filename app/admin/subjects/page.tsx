...
export default function SubjectsPage() {
  ...
  const fetchSubjects = async () => {
    ...
    if (error) {
      toast.error('حدث خطأ أثناء جلب المواد الدراسية')
    } else {
      setSubjects(data || [])
    }
    setLoading(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!name.trim()) return toast.error('يرجى إدخال اسم المادة')
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.error('لا يمكن الحذف لارتباط المادة ببيانات أخرى')
    }
  }

  const toggleStatus = async (subject: any, e: React.MouseEvent) => {
    e.stopPropagation()
    try {
      const { error } = await supabase
        .from('subjects')
  ...
  return (
    <div className="h-full flex flex-col max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-end mb-12">
        <div>
          <h1 className="text-4xl font-light tracking-tight text-gray-900 mb-3">المواد الدراسية</h1>
          <p className="text-gray-500 font-medium">إدارة وتنظيم المواد الدراسية عبر المنصة</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          >
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-6">
                      <div className={`px-4 py-1.5 rounded-full text-xs font-semibold flex items-center gap-2 transition-colors ${
                        ...
                        {subject.is_active ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                        {subject.is_active ? 'نشط' : 'معطل'}
                      </div>
                      
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        <button
                          onClick={(e) => toggleStatus(subject, e)}
            ...
            {subjects.length === 0 && (
              <div className="col-span-full h-40 flex items-center justify-center border border-dashed border-gray-200 rounded-2xl bg-white/50">
                <p className="text-gray-500">لا توجد مواد دراسية مضافة بعد.</p>
              </div>
            )}
          </motion.div>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-sm font-semibold text-gray-600 mb-2">اسم المادة</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                <div className="flex items-center justify-between p-5 bg-gray-50 rounded-2xl border border-gray-100">
                  <div className="flex flex-col">
                    <span className="text-gray-900 font-semibold">حالة المادة</span>
                    <span className="text-xs text-gray-500 mt-1">تفعيل أو تعطيل ظهور المادة للطلاب</span>
                  </div>
